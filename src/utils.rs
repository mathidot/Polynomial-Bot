...
pub mod crypto {
    ...
    {
        let decoded = URL_SAFE.decode(secret).map_err(|e|
            PolyfillError::config(format!("Invalid secret format: {}", e))
        )?;
        ...
        let mut mac = HmacSha256::new_from_slice(&decoded).map_err(|e|
            PolyfillError::internal("HMAC initialization failed", e)
        )?;
...
pub mod address {
    ...
    pub fn parse_address(addr: &str) -> Result<Address> {
        Address::from_str(addr).map_err(|e|
            PolyfillError::validation(format!("Invalid address format: {}", e))
        )
    }
    ...
    pub fn token_id_to_u256(token_id: &str) -> Result<U256> {
        ...
        U256::from_str_radix(token_id, 10).map_err(|e|
            PolyfillError::validation(format!("Invalid token ID: {}", e))
        )
    }
}
